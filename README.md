...
## 13. SQ-VAE（VQ-VAE系）および離散拡散モデル（D3PM）の実装

Sonyが発表している **SQ-VAE（VQ-VAE系）論文** を参考に、  
**512×512×3 の RGB 画像を、64×64 の空間解像度・コードブックサイズ 1024 の離散潜在表現に圧縮するモデル** を独自に実装。

### SQ-VAE モデルの成果
- 高解像度画像（512×512）を **離散潜在表現（64×64×1）にエンコード**
- その潜在表現からの再構成で **PSNR 約 28 dB** を達成
- 潜在表現を **可逆圧縮** することで、
  - 画質の劣化はごく僅か
  - **JPEG比で容量を約 1/400 に削減**
  - 見た目の違和感はほとんど発生しない高効率モデルを構築

### D3PM（離散拡散モデル）の実装と応用
SQ-VAE で得た離散潜在表現を用いるため、  
**D3PM（Discrete Denoising Diffusion Probabilistic Models）論文** を読み込み、潜在空間で動作する **離散拡散モデル（D3PM）を実装**。

- 潜在空間での拡散プロセスを構築し、高品質な画像生成を実現
- **[プロジェクトURL]** のように、特に **高品質な顔画像生成モデル** を完成
- 近々実装コードも載せる予定ですsq_vaeについてはすでに載せています
